...
for j in range(100):
    ...
    for l in range(4):        
        ...
        F1 = 1.0j*numpy.dot(U01inv,U01_d)
        F1 = 0.5 * (F1 + F1.H)
        G1,P1= LA.eig(F1)
        Q1 = numpy.dot(P1[:,0].H,U01inv)
        print norm(Q1)
        def calculate_mag_resp(Omega, U, P1, Q1):
            return numpy.abs(numpy.dot(Q1,numpy.dot(U, P1[:,0])))
        E=[]
        R=[]
        for Omega in t:
            print Omega
        ...
        print "R"
        print R
        filename1='First_frequency response few mode 1km 5 offset1 8'+str(j)+str(l)
        filename2 = 'Higher_frequency response few mode 1km 5 offset1 8'+str(j)+str(l)
        f1=open(filename1,'w')
        f2 = open(filename2,'w')
        pickle.dump(E,f1)
        pickle.dump(R,f2)
